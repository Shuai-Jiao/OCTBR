...
def find_associations_per_edge(ocel: OCEL, parameters: Optional[Dict[Any, Any]] = None) -> Dict[
    ...
    #print('ot1----','BELNR_EBELN_EBELP' in list(objtypes.values()))
    objtypes = {x: y[0] for x, y in objtypes.items()}
    #print('ot2----','BELNR_EBELN_EBELP' in list(objtypes.values()))
    #objtypes contains the missing ot
    history = {}
    edges = {}

    for evid in identifiers:
        #So the omap doesn't contain the object with that ot?
        #delete
        #Yes, actually there are many!
        '''if any([objtypes[obj]=='BELNR_EBELN_EBELP' for obj in omap[evid]]):
            print('Exist BELNR_EBELN_EBELP-----------')'''
        #delete
        for obj in omap[evid]:
            if obj in history:
                objtype = objtypes[obj]
                #Wait, so you mean, some object types are empty, and then\
                #they get lost here?
                #Checked the csv, not this case!
                #BUT is that mean the corresponding object only occur once???
                #then they cannot be catched up (no previous).
                #-----Yes
                #delete
                '''if objtype == 'BELNR_EBELN_EBELP':
                    print('Second BELNR_EBELN_EBELP----------')'''
                #delete
            ...
            #delete
            '''if objtypes[obj] == 'BELNR_EBELN_EBELP':
                    print('First BELNR_EBELN_EBELP----------')'''
            #delete
            history[obj] = evid
    #print('Whether in edge key:','BELNR_EBELN_EBELP' in edges.keys())
    return edges
