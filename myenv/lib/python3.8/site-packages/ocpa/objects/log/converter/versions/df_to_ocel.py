...
def apply(df: pd.DataFrame) -> ObjectCentricEventLog:
    #print('test getattr5',getattr(df.iloc[0], 'ocel:type:DEBIT_MEMO_DOC'))
    events = {}
    objects = {}
    acts = set()
    obj_names = set([x for x in df.columns if not x.startswith("event_")])
    #print('df columns',df.columns,'obj name',obj_names)
    val_names = set([x for x in df.columns if x.startswith(
        "event_")]) - set(['event_activity', 'event_timestamp', 'event_start_timestamp'])
    obj_event_mapping = {}
    #print('val name',val_names)
    start = time.time()
    #print('test getattr6',getattr(df.iloc[0], 'ocel:type:DEBIT_MEMO_DOC'))
    for index, row in enumerate(df.itertuples(), 1):
        #print('row attri:',row._fields)
        #print('get attr3:',getattr(row, 'ocel:type:DEBIT_MEMO_DOC'))
