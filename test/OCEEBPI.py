...
outputpath = parent_dir+'/test/output/BPIOCEE.txt'
...
filteredlist = [(1,2),(2,3),(3,10),(10,50)\
             ,(50,150),(150,550),(550,1550),\
                (2000,5000),(5000,15000),(15000,50000)]

    
for ele in filteredlist[:2]:
    path = prefix+f"BPI{ele[0]}to{ele[1]}executionprocess.jsonocel"
    ocel = ocel_import_factory.apply(path)
    print(ocel is None)
    #ocpn = ocpn_discovery_factory.apply(ocel, parameters={"debug": False})
    time0= time.time() 
    result,information =OC_escaping_edge(ocel,BPI0to1ocpn,cache=True)
    time1= time.time() 
