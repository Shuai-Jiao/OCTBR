...
outputpath = parent_dir+'/test/output/BPIcachedBST.txt'
...
'''outputpath = parent_dir+'/test/output/BPIcacheddebug.txt'
with open(outputpath, 'w') as file:
        file.write(f"-----Start caching debug-----")'''
...
for ele in filteredlist:
    ...
    #ocpn = ocpn_discovery_factory.apply(ocel, parameters={"debug": False})
    time0= time.time() 
    result,information = OCtokenbasedreplay2(ocel,BPI0to1ocpn,"BST",False,True)
    time1= time.time() 
    with open(outputpath, 'a') as file:
        file.write(f"-----Start {path}-----\n\
                   ----Backward replay OCTBR-------\
