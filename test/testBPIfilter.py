...
outputpath = parent_dir+'/test/output/BPIevaluate1.txt'
...
for ele in filteredlist:
    ...
    with open(outputpath, 'a') as file:
        file.write(f"-----Start {path}-----\n\
            ...
            Number of involving objects {len(info['object list'])}\n")
    time0 = time.time()
    fitness1 = OCtokenbasedreplay(BPI0to1ocpn,ocel)
    time1 = time.time()
    fitness2 = OCtokenbasedreplay(BPIfullocpn,ocel)
    time2 = time.time()
    print(f'OCTBR (with 0to1) is done, fitness:{fitness1}, time:{time1-time0}')
    print(f'OCTBR (with full) is done, fitness:{fitness2}, time:{time2-time1}')
    with open(outputpath, 'a') as file:
        file.write(f"------OCTBR------\n\
                OCTBR (with 0to1) is done, fitness:{fitness1}, time:{time1-time0}\n\
                OCTBR (with full) is done, fitness:{fitness2}, time:{time2-time1}\n")
    time3 = time.time()
    ...
    time5 = time.time()
    print(f"Ground Truth (with 0to1) is:{fitness3,precision3}, computation time:{time4-time3}\n\
