...
import address_token_flooding
import caching
import backward_replay.backward_replay as backward_replay_method
import backward_replay.variants.backward_searching_tree as backward_searching_tree
import backward_replay.variants.shortest_path as shortest_path

def token_based_replay(ocel,ocpn,token_flooding=None,cache=None):
    '''
    Return: a dictionary of the evaluation
    '''
    if token_flooding == None:
    ...
    obj_list = [(object_dict[object_id].type,object_id) for object_id in object_dict.keys()]
    ...
    for event_tuple in sorted_event_list:
        ...
        for ot in [pl.object_type for pl in preset]:
            if not any([ot==ele[0] for ele in obj_list]):
                ...
                information['unenabled_transitions'].add(curr_tr.name)
            ...
            if not need_backward_replay:
                ...
                if is_cached:                                       
                    ...
                    if cache['activity']:  
                        ...
                        caching_execution['missing_transition'] = curr_tr.name if m0 > 0 else None
                        key = mem
    ...
    #New tokens could be generated during the resolution of remaining tokens
    ...
    #result['is_missing_consistent'] = m == len(information['explicit_missing_tokens'])+len(information['implicit_missing_tokens'])
    #result['is_remaining_consistent'] = r == len(information['remaining_tokens'])
    #result['is_frozen_consistent'] = f == len(information['frozen_tokens'])
