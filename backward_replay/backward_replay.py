...
import backward_replay.variants.shortest_path as shortest_path_method
...
def shortest_path_backward_replay(ocpn,dg,marking,obj,parameters):
    ...
    while not found:
        ...
        for pair in search_pairs:
            try:
                time0 = time.time()
                found_path = shortest_path_method.find_path(dg,pair[0],pair[1])
                time1 = time.time()
        ...
        for places_path in sorted_paths_verification:
            time0 = time.time()
            verification,_ = shortest_path_method.verify(dg,marking,places_path,obj,parameters)
            time1 = time.time()
        ...
        if found:
            break
        silence_sequence = []
        shortest_path = shortest_executable_path
